...
<requirements>
- Local development server running (e.g., `npm run dev`, `next dev`)
...
|-------------|----------|
| `src/pages/users/*` or `app/routes/users/*` | `/users`, `/users/:id`, `/users/new` |
| `src/api/settings.ts` | `/settings` |
| `src/components/*` | Pages using that component |
